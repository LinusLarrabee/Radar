...
TargetVelocity(1:TargetNumber)=[50 10000 0 100];%目标径向速度 单位m/s   速度参数为[50 0 (Y*10+X+Z)*6 100]
...
% %-------------------------产生第4个目标的回波串-------%
   ...
   SignalTemp(DelayNumber(4)+1:DelayNumber(4)+number)=sqrt(SigPower(4))*exp(j*fi)*Chirp;
   %一个脉冲的1个目标（未加多普勒速度）
   Signal=zeros(1,TotalNumber);
   for i=1:PulseNumber
      Signal((i-1)*SampleNumber+1:i*SampleNumber)=SignalTemp;
...
% [fo,message] = fopen('/Users/sunhao/Desktop/Project0.1/coeff_fft_c.dat','wt');%频域脉压系数 
% if fo < 0
...
% fclose(fo); 
%  
% [fo,message]=fopen('/Users/sunhao/Desktop/Project0.1/echo_c.dat','wt');%16次回波的 
% if fo < 0
